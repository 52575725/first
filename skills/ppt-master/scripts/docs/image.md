...
Image tools cover visual asset planning, source-PPT image extraction, web image search, prompt-based AI generation, image inspection, and Gemini watermark removal.

## `visual_asset_planner.py`

Unified planner for the "should this paragraph use an image?" step.

```bash
# Dry run: extract source PPT/Markdown images, validate them, and write a plan
python3 scripts/visual_asset_planner.py <project_path>

# Execute planned acquisition: reuse good source images, search licensed web images,
# and generate custom images when configured
python3 scripts/visual_asset_planner.py <project_path> --execute

# Search only, no image generation
python3 scripts/visual_asset_planner.py <project_path> --execute --no-generate
```

Output:

- Extracted source images under `<project_path>/images/extracted/`
- Planned/executed visual decisions in `<project_path>/images/visual_asset_manifest.json`
- Web search results, when executed, under `<project_path>/images/visual_assets/` plus normal `image_sources.json`

Decision modes:

| Mode | Meaning |
|---|---|
| `existing` | Reuse a source PPT image only if validation says it is usable |
| `search` | Use `image_search.py` for concrete real-world subjects |
| `generate` | Use `image_gen.py` for abstract/custom scenes |
| `none` | Render as text/data/component without an image |

Validation uses Pillow when available and falls back to `ffprobe`; tiny thumbnails, unreadable files, and extreme aspect ratios are rejected.
